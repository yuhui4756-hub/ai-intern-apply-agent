...
当前不会自动点击投递、发送消息、自动填写页面或绕过验证码。自主询问模式只生成受轮次限制的待确认候选草稿；未匹配到岗位时会暂停并转为人工处理。全局暂停打开后，对话采集和浏览器巡检会跳过分析；重复采集同一段对话会被差分层跳过，不再调用模型，并在“最近巡检”里记录为无新内容。定时巡检目前只对受控 Edge 中已打开的聊天页做 dry-run 差分，不保存聊天全文，只保存哈希指纹。
